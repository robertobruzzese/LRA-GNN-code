        ...
        with torch.no_grad():
            ...
            for batch_idx, (embedding, age) in enumerate(dataloader):
                ...
                for i in range(x.size(0)):
                    ...
                    episode_return = 0.0  # per logging

                    for step in range(100):  # o max_steps
                        # --- STATO PULITO (come nel training): [embedding(512), pos_r, pos_c] = 514 ---
                        pos_tensor   = torch.tensor([[env.r / 10.0, env.c / 10.0]], device=device)
                        state_tensor = torch.cat([xi, pos_tensor], dim=1)  # 512 + 2 = 514

                        # Safety: verifica dim la prima volta
                        if step == 0:
                            assert state_tensor.shape[1] == self.q_network.fc1.in_features, \
                                f"State dim mismatch: got {state_tensor.shape[1]}, expected {self.q_network.fc1.in_features}"
                        ...
                        # Reward shaping lecito (la policy NON vede il target)
                    ...
                    total_samples += 1
                    total_reward += episode_return  # <<< accumula return episodio
