...
def train_prlae(agent, dataloader, device, dataset_name="MORPH", num_episodes=200, start_episode=0, save_every=50, best_accuracy=0.0,best_model_dir=None,classifier=None):  # 👈 nuovo parametro opzionale
    ...
    # Ricavato da agent.state_dim - 6 (perché aggiungiamo 6 valori nel state_tensor)
    embedding_dim = agent.state_dim - 6
    # Classificatore
   ...
   # else:
    ...
    for episode in range(start_episode, start_episode + num_episodes):
        ...
        for sample in tqdm(shuffled_loader, desc=f"Episode {episode + 1}/{num_episodes}"):

            if isinstance(sample, tuple):
                graph, age = sample
            else:
                graph = sample
                raise ValueError("⚠️ Atteso tuple (graph, age) dal dataloader")

            graph = graph.to(device)
            age = age.to(device)

            if not hasattr(graph, 'x') or graph.x is None:
                raise ValueError("❌ Il grafo non contiene 'x'")

            xi = graph.x.mean(dim=0, keepdim=True).detach()  # (1, embedding_dim)
            yi = age.view(-1)                                # (1,)
