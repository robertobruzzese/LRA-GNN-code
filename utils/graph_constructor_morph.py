...
def construct_initial_graph(csv_path, image_path, image_size, num_patches_x=6, num_patches_y=6, threshold_distance=0.9, tau_random_walk=None, embedding_dir="embeddings_morph"):
    ...
    emb_path = os.path.join(embedding_dir, "node_features.pt")
    edge_path = os.path.join(embedding_dir, "edge_features.pt")
...
def visualize_graph_diff(graph_initial, graph_augmented, patch_to_node, patches_tensor, save_path=None):
    import networkx as nx
    import matplotlib.pyplot as plt
    from torch_geometric.utils import to_networkx

    G_aug = to_networkx(graph_augmented, to_undirected=False)
    G_init = to_networkx(graph_initial, to_undirected=False)

    initial_edges = set([tuple(sorted(e)) for e in G_init.edges()])
    augmented_edges = set([tuple(sorted(e)) for e in G_aug.edges()])

    new_edges = augmented_edges - initial_edges

    image_height = 224
    pos = {
        patch_to_node[patch_id]: (
            patches_tensor[idx, 0].item(),
            image_height - patches_tensor[idx, 1].item()
        )
        for idx, patch_id in enumerate(patch_to_node.keys())
    }
    ...
    # Nodi
    nx.draw_networkx_nodes(G_aug, pos, node_color="red", node_size=300)

    # Archi originali (blu)
    nx.draw_networkx_edges(G_aug, pos,
        edgelist=list(initial_edges & augmented_edges),
        edge_color="blue", width=1.0, alpha=0.6,
        connectionstyle="arc3,rad=0.1"
    )

    # Archi nuovi (rossi tratteggiati)
    nx.draw_networkx_edges(G_aug, pos,
        edgelist=list(new_edges),
        edge_color="red", style="dashed", width=1.5, alpha=0.9,
        connectionstyle="arc3,rad=0.25"
    )

    # Etichette nodi
    nx.draw_networkx_labels(G_aug, pos,
        labels={patch_to_node[patch_id]: patch_id for patch_id in patch_to_node.keys()},
        font_size=10, font_weight='bold', font_color="white"
    )

    plt.title("📌 Differenze tra grafo iniziale e aumentato")
    plt.axis('off')
    if save_path:
