...
def clean_and_load_q_state(agent, state_raw, device):
    """
    Accetta:
      - puro state_dict del Q (chiavi 'fc1.weight', ...)
      - dict{'q_network_state_dict': ...} o {'state_dict': ...}
      - chiavi prefissate 'q_network.xxx'
    """
    ...
    if "fc1.weight" in clean_state:
        in_features_ckpt = clean_state["fc1.weight"].shape[1]
...
run_name = f"{dataset_name}_clean514"    # cartella dei ckpt “puliti 514”
...
# Prendi gli ULTIMI K mantenendo l’ordine (oldest → newest tra gli ultimi K)
sel = candidates[-RECENT_K:] if len(candidates) > RECENT_K else candidates
ckpt_list = [p for (p, _ts, _base) in sel]
# ==== PRIORITÀ AI best_agent_* ====
best_mae_files = sorted(
    glob.glob(os.path.join(checkpoint_dir, "best_agent_mae_*.pth")),
    key=os.path.getmtime
)
best_acc_files = sorted(
    glob.glob(os.path.join(checkpoint_dir, "best_agent_acc_*.pth")),
    key=os.path.getmtime
)
using_best = False
if best_mae_files:
    print(f"🔍 Trovati {len(best_mae_files)} best_agent_mae_* → li valuto in priorità.")
    # se vuoi limitarli agli ultimi K:
    ckpt_list = best_mae_files[-RECENT_K:] if len(best_mae_files) > RECENT_K else best_mae_files
    using_best = True
elif best_acc_files:
    print(f"🔍 Trovati {len(best_acc_files)} best_agent_acc_* → li valuto in priorità.")
    ckpt_list = best_acc_files[-RECENT_K:] if len(best_acc_files) > RECENT_K else best_acc_files
    using_best = True
# =====================================
if using_best:
    print(f"📦 Valuto {len(ckpt_list)} checkpoint BEST (by mtime, oldest → newest):")
