...
class AgeEstimationDatasetFGNET(Dataset):
    def __init__(self, root_dir, dataset_name="MORPH", embedding_split="train", transform=None,
        ...
        for fname in os.listdir(self.embeddings_dir):
            ...
            try:
                ...
                # DOPO (uniforme a CLAP2016: DFE usa SEMPRE graph_rw.pt)
    ...
    def __getitem__(self, idx):
        ...
        # === Caso 3: DFE only (usa deep_features.pt)
            ...
            g_path = os.path.join(self.embeddings_dir, image_name, "graph_rw.pt")
            g = torch.load(g_path)

            assert isinstance(g, Data), f"❌ graph_rw.pt deve essere un oggetto Data, trovato: {type(g)}"
            assert hasattr(g, "x"), "❌ Il grafo non contiene il campo x"
            assert g.x.size(1) == 512, f"❌ Feature dim errata: {g.x.size(1)}, atteso 512"
