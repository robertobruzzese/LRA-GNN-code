...
from utils.graph_constructor_utkface import construct_initial_graph
from utils.lrc import LatentRelationCapturer
from utils.deep_feature_extraction import extract_deep_features

# === Config generale (allineata a FGNET) ===
splits = [("Train", "train"), ("Validation", "val")]  # (nome cartella reale, nome out)
base_dir = "datasets/data/UTKFace/images"
embeddings_root = "embeddings_utkface"
...
# se =0 non crea LRC, con fallback a RW
num_heads = 8

# ResGCN / estrattore deep
num_layers = 4
...
for split_name, split_out in splits:
    ...
    for image_filename in image_files:
        ...
        out_dir    = os.path.join(embeddings_root, split_out, image_id)
        ...
        graph_initial, graph_rw, patches_tensor, patch_to_node = result
        torch.save(graph_initial,  os.path.join(out_dir, "graph_initial.pt"))
        torch.save(graph_rw,        os.path.join(out_dir, "graph_rw.pt"))
        torch.save(patches_tensor,  os.path.join(out_dir, "patches_tensor.pt"))
        torch.save(patch_to_node,   os.path.join(out_dir, "patch_to_node.pt"))
        print("💾 Grafi iniziale e RW salvati.")

        # === Step 2: LRC (se num_heads > 0), altrimenti si userà RW in fallback
        X = graph_rw.x
        if num_heads and num_heads > 0:
            try:
                ...
                for idx, A_m in enumerate(A_m_list):
                    ...
                    torch.save(graph_m, os.path.join(out_dir, f"graph_lrc_{idx}.pt"))
        ...
        try:
            graph_paths = sorted(glob.glob(os.path.join(out_dir, "graph_lrc_*.pt")))
            if not graph_paths:
                rw_path = os.path.join(out_dir, "graph_rw.pt")
            ...
            torch.save(deep_feats, os.path.join(out_dir, "deep_features.pt"))
